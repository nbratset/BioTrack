...
def barplot_taxa_facet_fill(otu_table, taxonomy_series, metadata,
    ...
    Parameters:
        ...
        metadata: DataFrame, sample metadata,
                  index=samples, includes 'Condition'
    ...
    top_tax = otu_by_tax.sum().sort_values(ascending=False).head(top_n).index.tolist()  # noqa
    otu_top = otu_by_tax[top_tax].copy()
    otu_top["Other"] = otu_by_tax.drop(columns=top_tax, errors="ignore").sum(axis=1)  # noqa
    ...
    df_melt = otu_top_rel.melt(id_vars=["Sample", "Condition"],
                               var_name="Taxa",
                               value_name="Proportion")

    # Pivot for easier plotting
    taxa_order = top_tax + ["Other"]
    pivot_df = df_melt.pivot_table(index=["Sample", "Condition"],
                                   columns="Taxa",
    ...
    fig, axes = plt.subplots(nconds, 1,
                             figsize=(fig_width, height_per_condition*nconds),
                             sharey=True)
    ...
    for i, (ax, condition) in enumerate(zip(axes, conditions)):
        ...
        sorted_samples = cond_df[most_abundant_genus].sort_values(ascending=False).index  # noqa
